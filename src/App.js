...
function App() {
  ...
  const ToggleBtnStyle = "py-2 px-4 rounded-lg font-medium transition-colors";
  const KanbanviewStyle =
    currentView === "kanban"
      ? "bg-olive-green-600 text-white shadow-md"
      : "bg-spearmint-200 text-olive-green-700 hover:bg-spearmint-300";
  const MatrixviewStyle =
    currentView === "matrix"
      ? "bg-olive-green-600 text-white shadow-md"
      : "bg-spearmint-200 text-olive-green-700 hover:bg-spearmint-300";
          ...
          <>
            ...
            <div className="my-6 flex justify-center space-x-2">
              <button
                ...
                className={`${ToggleBtnStyle} ${KanbanviewStyle}`}
              >
                Kanban View
              </button>
              <button
                onClick={() => setCurrentView("matrix")}
                className={`${ToggleBtnStyle} ${MatrixviewStyle}`}
              >
                Matrix View
              </button>
            </div>
            ...
            {!isLoading && tasks.length > 0 && currentView === "kanban" && (
            ...
            {!isLoading && tasks.length > 0 && currentView === "matrix" && (
              <EisenhowerMatrix
                tasks={tasks}
                onViewTask={openViewTaskModal}
                // Pass other handlers if your matrix task items become more interactive
                // onEditRequest={openEditTaskModal}
                // onDeleteRequest={requestDeleteConfirmation}
              />
            )}
